"""
  ...
  Author:  Your professors (for the framework and lower-level code)
    and Jack Franey, Ezrie McCurry, and Loki Strain.
...
class DriveSystem(object):
    ...
    def go_straight_for_inches_using_encoder(self, inches, speed):
        ...
        self.go(speed, speed)
        print((int(inches) * (360 / self.wheel_circumference)))
        while True:
            print(self.left_motor.get_position())
            if self.left_motor.get_position() >= (int(inches) * (360 / self.wheel_circumference)):
                self.stop()
                break
    ...
    def go_backward_until_distance_is_greater_than(self, inches, speed):
        ...
        inf = self.sensor_system.ir_proximity_sensor
        self.go(-speed, -speed)
        while True:
            if inf.get_distance_in_inches() > inches:
                self.stop()
                break
    ...
    def go_until_distance_is_within(self, inches, speed, delta):
        ...
        if inf.get_distance_in_inches() > maxd:
            self.go(speed, speed)
            while True:
                if inf.get_distance_in_inches() <= maxd:
                    self.stop()
        ...
        if inf.get_distance_in_inches() < mind:
            self.go(-speed, -speed)
            while True:
                if inf.get_distance_in_inches() >= mind:
                    self.stop()
    ...
    def spin_clockwise_until_sees_object(self, speed, area):
        ...
        self.left_motor.turn_on(speed)
        while True:
            if int(pixy.value(3)) * int(pixy.value(4)) >= area and 150 < int(pixy.value(1)) < 170:
                self.right_motor.turn_off()
                self.left_motor.turn_off()
                break
